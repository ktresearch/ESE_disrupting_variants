...
foreach my $cancertype (@cancertypelist) {
	...
	my $rpkm_non_variant_file = "$workdir/$cancertype[1]/permutationtest/rpkm_non_variant_exon.txt";
	...
	my $datadir = "$workdir/$cancertype[1]/permutationtest/skip_data";
	opendir (DIR, "$datadir");
	my @files = readdir(DIR);
	closedir (DIR);

	my $outputall = "$workdir/$cancertype[1]/permutationtest/ese_judge_all.txt";
	my $outputese = "$workdir/$cancertype[1]/permutationtest/ese_judge.txt";
	open (ALL, ">$outputall");
	open (ESE, ">$outputese");
